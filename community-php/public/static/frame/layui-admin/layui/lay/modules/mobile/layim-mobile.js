...
layui.define(['laytpl', 'upload-mobile', 'layer-mobile', 'zepto'], function(exports){
  ...
  var elemTpl = ['<div class="layui-layim">'
    ...
    ,'<div class="layim-tab-content">'
      ...
      ,'<ul class="layim-list-friend">'
        ...
        ,'<li>'
          ,'<index layim-event="spread" lay-type="{{ spread }}"><i class="layui-icon">{{# if(spread === "true"){ }}&#xe61a;{{# } else {  }}&#xe602;{{# } }}</i><span>{{ item.groupname||"未命名分组"+index }}</span><em>(<cite class="layim-count"> {{ (item.list||[]).length }}</cite>)</em></index>'
  ...
  layui.data.content = function(content){
    ...
    var html = function(end){
      return new RegExp('\\n*\\['+ (end||'') +'(pre|div|p|table|thead|th|tbody|tr|td|ul|li|ol|li|dl|dt|dd|h2|h3|h4|index)([\\s\\S]*?)\\]\\n*', 'g');
    };
